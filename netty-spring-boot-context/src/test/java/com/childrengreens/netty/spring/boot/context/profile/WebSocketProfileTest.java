...
/**
 * Tests for {@link WebSocketProfile}.
 */
class WebSocketProfileTest {

    private WebSocketProfile profile;
    private ServerSpec serverSpec;

    @BeforeEach
    void setUp() {
        profile = new WebSocketProfile();
        serverSpec = new ServerSpec();
        serverSpec.setName("test-server");
    }

    @Test
    void getName_returnsWebsocket() {
        assertThat(profile.getName()).isEqualTo("websocket");
    }
