...
/**
 * Tests for {@link TcpTransportStarter}.
 */
class TcpTransportStarterTest {

    private TransportFactory transportFactory;
    private TcpTransportStarter tcpTransportStarter;
    ...
    void setUp() {
        ...
        tcpTransportStarter = new TcpTransportStarter(transportFactory);
    ...
    void start_withValidSpec_createsRuntime() throws Exception {
        ...
        serverSpec.setName("test-tcp-server");
        ...
        serverRuntime = tcpTransportStarter.start(serverSpec, bossGroup, workerGroup, initializer);
    ...
    void start_withoutHost_bindsToAllInterfaces() throws Exception {
        int port = findAvailablePort();
        ServerSpec serverSpec = new ServerSpec();
        serverSpec.setName("test-tcp-server-no-host");
        serverSpec.setHost("");
        serverSpec.setPort(port);
    ...
    void start_withDefaultHost_bindsToAllInterfaces() throws Exception {
        int port = findAvailablePort();
        ServerSpec serverSpec = new ServerSpec();
        serverSpec.setName("test-tcp-server-default");
    ...
    void start_withNullHost_bindsToAllInterfaces() throws Exception {
        int port = findAvailablePort();
        ServerSpec serverSpec = new ServerSpec();
        serverSpec.setName("test-tcp-server-null-host");
    ...
    void constructor_setsTransportFactory() {
        TcpTransportStarter starter = new TcpTransportStarter(transportFactory);
