...
<3> Action when limit exceeded: `DROP`, `CLOSE`, or `REJECT` (default: `DROP`)

=== Rate Limit Actions

The `action` property controls what happens when the rate limit is exceeded:
...
|`DROP`
|Drop the message silently, keep connection open
|Long-lived connections (WebSocket, TCP) where occasional drops are acceptable

|`CLOSE`
|Close the connection after dropping the message
|Strict enforcement, punish abusive clients

|`REJECT`
|Send an error response, keep connection open
|Client-friendly, allows retry with backoff
|===

NOTE: For HTTP protocol, a `429 Too Many Requests` response is always sent regardless of the action setting.

=== Protocol-Specific Behavior

[cols="1,2,2,2"]
|===
|Protocol |DROP |CLOSE |REJECT
...
|WebSocket
|Drop message, keep open
|CloseFrame(1008) + close
|Send error frame, keep open

|TCP
|Drop message, keep open
|Close connection
|Send error JSON, keep open
