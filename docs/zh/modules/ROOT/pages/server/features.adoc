...
<1> 持续速率（每秒添加的令牌数）
<2> 最大突发量（桶容量）
<3> 超限时的动作：`DROP`、`CLOSE` 或 `REJECT`（默认：`DROP`）

=== 限流动作

`action` 属性控制超过速率限制时的行为：
...
NOTE: 对于 HTTP 协议，无论 action 设置如何，始终返回 `429 Too Many Requests` 响应。

=== 协议特定行为

[cols="1,2,2,2"]
|===
|协议 |DROP |CLOSE |REJECT
